...
class UserManagementTest : TestBase() {
  ...
  @Test
  fun `authenticateWithRadarEmailChallenge returns a typed response`() {
    stubResponse(
      "POST",
      "/user_management/authenticate",
      200,
    ...
    val result = api().authenticateWithRadarEmailChallenge("sample-arg", "sample-arg", "sample-arg")
    assertNotNull(result)
    assertEquals("sample", result.accessToken)
    assertEquals("sample", result.refreshToken)
  }
  ...
  @Test
  fun `authenticateWithRadarSmsChallenge returns a typed response`() {
    stubResponse(
      "POST",
      "/user_management/authenticate",
      200,
  ...
  fun `createCorsOrigin returns a typed response`() {
    ...
    assertNotNull(result)
    assertEquals("cors_origin", result.objectType)
    assertEquals("sample", result.id)
  ...
  fun `getEmailVerification returns a typed response`() {
    ...
    assertNotNull(result)
    assertEquals("email_verification", result.objectType)
    assertEquals("sample", result.id)
    assertEquals("sample", result.userId)
    assertEquals("sample", result.email)
  ...
  fun `resetPassword returns a typed response`() {
    ...
    assertNotNull(result)
    assertEquals("password_reset", result.objectType)
    assertEquals("sample", result.id)
    assertEquals("sample", result.userId)
    assertEquals("sample", result.email)
  ...
  fun `update returns a typed response`() {
    ...
    assertEquals(false, result.emailVerified)
    wireMockRule.verify(
      putRequestedFor(urlPathMatching("/user_management/users/sample-arg"))
        .withQueryParam("password", absent())
        .withQueryParam("password_hash", absent())
        .withQueryParam("password_hash_type", absent())
    )
  ...
  fun `sendEmailChange returns a typed response`() {
    ...
    assertNotNull(result)
    assertEquals("email_change", result.objectType)
  ...
  fun `sendInvitation returns a typed response`() {
    ...
    assertNotNull(result)
    assertEquals("invitation", result.objectType)
    assertEquals("sample", result.id)
    assertEquals("sample", result.email)
  ...
  fun `createRedirectUri returns a typed response`() {
    ...
    assertNotNull(result)
    assertEquals("redirect_uri", result.objectType)
    assertEquals("sample", result.id)
